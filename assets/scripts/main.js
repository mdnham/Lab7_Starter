...
function createRecipeCards() {
  for(var i = 0; i < recipes.length; i++) {
    ...
    // Add 'hidden' class to recipes past index 2
    if(i > 2)
    ...
    router.addPage(page, function() {
      ...
      for(var ind = 0; ind < recipes.length; ind++) {
        if(recipeData[recipes[ind]]['page-name'] == page)
          break;
      }
      document.querySelector('recipe-expand').data = recipeData[recipes[ind]];
    });
